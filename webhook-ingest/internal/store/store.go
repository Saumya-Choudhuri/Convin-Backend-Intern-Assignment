...
// InsertEvent stores the raw delivery if this event_id has not been seen
// before. It returns true when the row was newly inserted and false when the
// event was already deduplicated by the database.
func (s *Store) InsertEvent(ctx context.Context, e Event) (bool, error) {
	...
	err := s.pool.QueryRow(ctx,
		`INSERT INTO events (event_id, call_id, account_id, payload)
		 ...
		 RETURNING event_id`,
		e.EventID, e.CallID, e.AccountID, e.Payload).Scan(&eventID)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, nil
	}
	if err != nil {
