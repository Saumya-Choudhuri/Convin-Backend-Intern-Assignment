...
func TestConcurrentDuplicateDeliveryIsIgnored(t *testing.T) {
	...
	if err := st.Pool().QueryRow(ctx, `SELECT call_count FROM account_stats WHERE account_id = $1`, accountID).Scan(&statsCount); err != nil {
		t.Fatalf("scan account_stats: %v", err)
	}
	if statsCount != 1 {
		t.Fatalf("account_stats call_count = %d, want 1", statsCount)
	}
}
