...
func (s *Service) Ingest(ctx context.Context, evt Event) error {
	...
	// Do not inherit the HTTP request context: requests end as soon as we respond,
	// which would cancel any in-flight processing and make it silently disappear.
	if rec.RecordingURL != "" {
		go func() {
...
func (s *Service) processRecording(rec store.Event) error {
	...
	return s.store.MarkRecordingProcessed(procCtx, rec.CallID)
}
